...
Web-based terminal multiplexer for [Claude Code](https://docs.anthropic.com/en/docs/claude-code), [Codex](https://github.com/openai/codex), Gemini CLI, pi, and [Grok Build](https://x.ai/cli) sessions.
...
- **Project sidebar** — all projects with Claude Code, Codex, Gemini CLI, pi, and/or Grok Build history, merged by directory
...
- **Terminal tabs** — multiple concurrent Claude Code / Codex / Gemini / pi / Grok terminals with tab management
...
- Scans `~/.pi/agent/sessions/` for pi session JSONL files
- Scans `~/.grok/sessions/` for Grok Build sessions (`summary.json` + `updates.jsonl`)
...
- **Agents**: Claude Code via `claude` CLI, Codex via `codex` CLI, Gemini via `gemini` CLI, pi via `pi` CLI, Grok Build via `grok` CLI
