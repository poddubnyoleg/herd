...
- Local model button: `⌁` in the sidebar header (hidden unless the `gemma-server` launcher script exists). Gray = down, click to start (pulses while llama.cpp loads the model, polls up to 2 min), green = `/health` responding, click while green = stop (with confirm)
